...
linked_list *execute(char *command, size_t n, linked_list *database) {
    char *key;
    char *value;
    int i = 0;
    int value_cpt = 0;
...
int main () {
    linked_list *database = malloc(sizeof(linked_list));
    char *line = NULL;
    size_t n = 0;
    database = NULL;

    while(getline(&line, &n, stdin) != -1) {
        database = execute(line, n, database);
        free(line);
        line = NULL;
